/**
 * Defer Forms — multi-step forms. Splits a form into steps at each `.deferforms-pagebreak`
...
( function () {
	...
	var l10n = window.deferformsStepsL10n || {};
	var TEXT = {
		prev:   l10n.prev   || 'Back',
		next:   l10n.next   || 'Next',
	...
	function stepKey( form ) {
		...
		return 'deferforms_step_' + ( tag || 'form' );
	}
	...
	function firstBadStep( steps ) {
		if ( ! window.deferformsValidate ) {
			return -1;
		}

		for ( var i = 0; i < steps.length; i++ ) {
			if ( window.deferformsValidate.container( steps[ i ] ) ) {
				return i;
			}
		}
	...
	function build( form ) {
		if ( form.dataset.deferformsSteps || ! form.querySelector( '.deferforms-pagebreak' ) ) {
			return;
		}
		form.dataset.deferformsSteps = '1';
		...
		Array.prototype.slice.call( form.children ).forEach( function ( node ) {
			if ( node.classList && node.classList.contains( 'deferforms-pagebreak' ) ) {
		...
		var steps = groups.map( function ( group ) {
			...
			step.className = 'deferforms-step' + ( group.settings.class ? ' ' + group.settings.class : '' );
		...
		var bar  = el( 'div', 'deferforms-steps-progress' );
		var fill = el( 'div', 'deferforms-steps-progress-fill' );
		bar.appendChild( fill );

		var marks = el( 'ol', 'deferforms-steps-marks' );
		var dots  = groups.map( function ( group, index ) {
			var mark  = el( 'li', 'deferforms-steps-mark' );
			var num   = el( 'span', 'deferforms-steps-mark-num' );
			var label = el( 'span', 'deferforms-steps-mark-label' );
		...
		var indicator = el( 'div', 'deferforms-steps-indicator' );
		indicator.appendChild( bar );
		indicator.appendChild( marks );
		steps[ 0 ].parentNode.insertBefore( indicator, steps[ 0 ] );

		var nav    = el( 'div', 'deferforms-steps-nav' );
		var prev   = btn( 'deferforms-step-prev', TEXT.prev );
		var status = el( 'span', 'deferforms-step-status' );
		var next   = btn( 'deferforms-step-next', TEXT.next );
		...
		function show( index, scroll, remember ) {
			...
			steps.forEach( function ( step, idx ) {
				step.classList.toggle( 'deferforms-step-active', idx === current );
			} );
			...
			dots.forEach( function ( mark, idx ) {
				mark.classList.toggle( 'deferforms-steps-mark-done', idx < current );
				mark.classList.toggle( 'deferforms-steps-mark-current', idx === current );
		 ...
		 */
		window.deferforms.onReset( form, function () {
			if ( sent ) {
				sent = false;
				return;
			}
	...
	function validStep( step ) {
		...
		var first = window.deferformsValidate.container( step );
		if ( ! first ) {
			return true;
		}
	...
	function heading( settings ) {
		var box = el( 'div', 'deferforms-step-heading' );

		if ( settings.title ) {
			var title = el( 'h3', 'deferforms-step-title' );
			title.textContent = settings.title;
			box.appendChild( title );
		}

		if ( settings.desc ) {
			var desc = el( 'p', 'deferforms-step-desc' );
	...
	function btn( className, label ) {
		var button = el( 'button', 'deferforms-step-btn ' + className, label );
		button.type = 'button';
		return button;
	}

	window.deferforms.forms( build );
} )();
