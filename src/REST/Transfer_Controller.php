...
final class Transfer_Controller extends Controller {
	...
	public function rest_export( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( empty( $ids ) ) {
			return self::error( 'no_forms', __( 'No forms were chosen to export.', 'cf7-nova-lite' ), 400 );
		}
		...
		if ( empty( $forms ) ) {
			return self::error( 'no_forms', __( 'None of the chosen forms could be read.', 'cf7-nova-lite' ), 404 );
		}
	...
	public function rest_import( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! is_array( $bundle ) ) {
			return self::error( 'not_a_bundle', __( 'That file is not a Nova form bundle.', 'cf7-nova-lite' ), 400 );
		}

		$forms = Form_Bundle::unpack( $bundle );

		if ( empty( $forms ) ) {
			return self::error( 'nothing_to_import', __( 'The bundle holds no forms this version can read.', 'cf7-nova-lite' ), 400 );
		}
