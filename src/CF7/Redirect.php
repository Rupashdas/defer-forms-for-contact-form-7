 ...
 *
 * The settings live in one `_deferforms_redirect` post meta and are handed to the
...
final class Redirect {
	...
	private const META = '_deferforms_redirect';
	...
	public function inject( string $elements ): string {
		...
		wp_enqueue_script( 'deferforms-redirect', DEFERFORMS_URL . 'assets/js/redirect.js', array(), deferforms_asset_ver( 'assets/js/redirect.js' ), true );

		return $elements . sprintf(
			'<input type="hidden" class="deferforms-redirect" value="%1$s" data-delay="%2$d" data-target="%3$s" data-method="%4$s" data-params="%5$s" />',
