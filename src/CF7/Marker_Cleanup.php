 ...
 *    visitor as literal `[deferforms_row]` text is bad enough to keep a net under.
 *
 * @package DEFERFORMS
 */
...
final class Marker_Cleanup {
	...
	public function tidy( string $elements ): string {
		if ( false !== strpos( $elements, '[deferforms_' ) ) {
			$elements = $this->strip_markers( $elements );
		}

		if ( false !== strpos( $elements, 'deferforms-' ) ) {
	 ...
	 * where the layout used to be. The form does not print `[deferforms_row]` at
