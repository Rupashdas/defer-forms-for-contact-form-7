...
final class Product_Field {
	...
	public function render( $tag ): string {
		...
		if ( ! self::is_available() ) {
			return '<span class="cf7nl-product-missing">'
				. esc_html__( 'WooCommerce is not active, so there are no products to list.', 'cf7-nova-lite' )
				. '</span>';
		}

		$options = '<option value="">' . esc_html__( 'Select a product…', 'cf7-nova-lite' ) . '</option>';
		...
		$class = $tag->get_class_option( wpcf7_form_controls_class( $tag->type ) ) . ' cf7nl-product';
