...
final class Form_Serializer {
	...
	private static function serialize_field( array $field ): string {
		...
		// date field also carries the cf7nl-fp marker the front-end picker reads.
		...
		if ( $labelable && '' !== $label ) {
			...
			if ( in_array( $type, array( 'radio', 'checkbox', 'file', 'rating' ), true ) ) {
				return self::wrap_condition( $field, "<fieldset class=\"nv-fieldset\"><legend>{$label}</legend>\n{$tag}</fieldset>" );
			}
	...
	/**
	 * Wrap a field's markup in a `[cf7nl_if]` region when it carries a condition.
	...
	private static function wrap_condition( array $field, string $html ): string {
		...
		return "[cf7nl_if action=\"{$action}\" groups=\"{$encoded_groups}\"]\n{$html}\n[/cf7nl_if]";
	}

	/**
	 * A grid row → `[cf7nl_row cols="N"][cf7nl_col] … [/cf7nl_col] … [/cf7nl_row]`.
	...
	private static function serialize_row( array $row ): string {
		...
		return '[cf7nl_row cols="' . count( $columns ) . "\"]\n" . implode( "\n", $cells ) . "\n[/cf7nl_row]";
	}
	...
	private static function serialize_content( array $item ): string {
		...
		if ( 'spacer' === $type ) {
			...
			return "<div class=\"nv-spacer\" style=\"height:{$height}px\" aria-hidden=\"true\"></div>";
		}
		return '';
	}
	...
	private static function serialize_step( array $item ): string {
		$tag_parts = array( 'cf7nl_pagebreak' );
